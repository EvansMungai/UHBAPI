...
public class StudentRoutes : IRouteRegistrar
{
    ...
    private void MapStudentRoutes(WebApplication webApplication)
    {
        ...
        app.MapGet("/student", (StudentHandler handler, string id) => handler.GetStudentById(id)).WithTags("Students").Produces(200).Produces(404).Produces<StudentDto>().RequireAuthorization("CanAccessEverything");
