...
public class ApplicationRoutes : IRouteRegistrar
{
    ...
    public void MapApplicationRoutes(WebApplication application)
    {
        ...
        app.MapGet("/user-applications", (ApplicationHandler handler, string id) => handler.GetUserApplication(id)).WithTags("Applications").Produces(200).Produces(404).Produces<List<ApplicationDto>>().RequireAuthorization("CanAccessEverything");
        ...
        app.MapGet("/application/{id}", (ApplicationHandler handler, int id) => handler.GetApplicationById(id)).WithTags("Applications").Produces(200).Produces(404).Produces<ApplicationDto>().RequireAuthorization("CanAccessApplicationDetails");
